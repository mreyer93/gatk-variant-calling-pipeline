...
# foo <- foo[!duplicated(foo$ensembl_gene_id),]
# rownames(foo) <- foo$ensembl_gene_id
# foo[foo$external_transcript_name=='',"external_transcript_name"] <- foo[foo$external_transcript_name=='',"ensembl_gene_id"]
# rownames(cts) <- foo[gene.ids, "external_transcript_name"]
...
cts.agg <- aggregate.data.frame(cts[,1:12], by=list(cts$gene), FUN= sum)
rownames(cts.agg) <- unique(cts.agg$Group.1)
cts.agg <- cts.agg[,2:13]
...
outf.counts <- '~/pcloud_sync/project/rnaseq/jak2_experiment/results/counts/normalized_counts_export.tsv'
...
outf.counts.mean <- '~/pcloud_sync/project/rnaseq/jak2_experiment/results/counts/normalized_counts_group_mean.tsv'
...
out.interaction <- data.frame(name = rownames(resLFCOrdered.interaction),
                           description = gene.to.desc[rownames(resLFCOrdered.interaction)],
...
dds <- DESeqDataSetFromMatrix(countData=cts.agg[, rownames(coldata)[coldata$genotype=='JAK2V617F']],
...
#     lsd1 in wt mice      ####################################################
###############################################################################
dds <- DESeqDataSetFromMatrix(countData=cts.agg[, rownames(coldata)[coldata$genotype=='Bl6_wt']],
...
###############################################################################
dds <- DESeqDataSetFromMatrix(countData=cts.agg[, rownames(coldata)[coldata$condition=='Vehicle']],
...
###############################################################################
dds <- DESeqDataSetFromMatrix(countData=cts.agg[, rownames(coldata)[coldata$condition=='LSDi']],
