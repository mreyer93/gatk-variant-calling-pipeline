# investigate the project CTP_101 data that was parsed with python and loaded up in R
gen_df <- read.table('~/local_data/p6/investigate_sample_data/CTP_101/p6_parsed_haplotypecaller.tsv', 
...
metadata <- read.table('~/local_data/p6/investigate_sample_data/CTP_101/CTP_101_metadata.tsv', sep='\t', quote='', header=T, comment.char = '')
...
table(metadata$p6_positive, useNA = 'ifany')

table(metadata$p6_positive, metadata$JAK2_V617F, useNA = 'ifany')
...
table(metadata$p6_positive, metadata$any_chr_event, useNA = 'ifany')
...
write.table(metadata, '~/local_data/p6/investigate_sample_data/CTP_101/CTP_101_metadata_p6.tsv', sep='\t', quote=F, row.names = F, col.names = T)
write.table(metadata, '~/cloud_storage/shared_data/shared_p6/CTP_101_metadata_p6.tsv', sep='\t', quote=F, row.names = F, col.names = T)
