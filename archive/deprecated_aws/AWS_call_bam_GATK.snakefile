...
rule all: 
    input: 
        # expand(join(outdir, '01_prepare_bam/recalibrate/{sample}.fixchr.bam'), sample=sample_list),
        # S3.remote(expand(join('data-bucket/gatk_calling', outdir, '01_prepare_bam/recalibrate/{sample}.fixchr.bam'), sample=sample_list)),
        # expand(join(outdir, '01_prepare_bam/GetPileupSummaries/{sample}_GetPileupSummaries.table'), sample=sample_list),
        # expand(join(outdir, '01_prepare_bam/CalculateContamination/{t_sample}_contam.table'), t_sample=all_t_contamination_samples),
        # expand(join(outdir, '02_variants_reference/filtered/{sample}_filtered.vcf'), sample=sample_list),
        # expand(join(outdir, '02_variants_reference_exome/filtered/{sample}_filtered.vcf'), sample=sample_list),
...
rule add_read_groups:
    input: lambda wildcards: S3.remote(bam_map[wildcards.EGAF])
    output: temporary(join(outdir, '01_prepare_bam/add_readgroups/{EGAF}.bam'))
    params:
        RGSM = lambda wildcards: metadata.loc[wildcards.EGAF, "sample"],
        RGLB = lambda wildcards: metadata.loc[wildcards.EGAF, 'library'],
        RGPL = lambda wildcards: metadata.loc[wildcards.EGAF, 'platform'],
        RGPU = lambda wildcards: metadata.loc[wildcards.EGAF, 'platform_unit'],
    shell: """
...
# fix chromosome annotations
rule fix_chromosomes:
    input: rules.add_read_groups.output
    output: temporary(join(outdir, '01_prepare_bam/add_readgroups/{EGAF}.fixchr.bam'))
    shell: """
...
# used in the AWS implementation becasue mark dups has not been run
# (to my knowledge on the EGA data)
# in this step we also combine the multiple replicates from each sample
rule mark_duplicates:
    input: 
        bams = lambda wildcards: expand(join(outdir, '01_prepare_bam/add_readgroups/{EGAF}.fixchr.bam'), EGAF = sample_to_EGAF[wildcards.sample])
    output: 
        bam = temporary(join(outdir, '01_prepare_bam/add_readgroups/{sample}.mkdup.bam')),
        # stats = join(outdir, '01_prepare_bam/add_readgroups/{sample}_mkdup_metrics.txt')
    threads: 8
    params:
        input_string = lambda wildcards: ' -I '.join(expand(join(outdir, '01_prepare_bam/add_readgroups/{EGAF}.fixchr.bam'), EGAF = sample_to_EGAF[wildcards.sample])),
        tmpdir = "/fasttmp"
...
rule bam_index: 
    input: rules.mark_duplicates.output
    output: join(outdir, '01_prepare_bam/add_readgroups/{sample}.mkdup.bam.bai'),
    shell: """
...
rule copy_final_bam_remote:
    input: 
        bam = rules.bqsr.output.bam,
        bai = rules.bqsr.output.bai,
    ...
    output:
        ...
        table = S3.remote(join('data-bucket/gatk_calling', outdir, '01_prepare_bam/recalibrate/{sample}.table')),
    shell: """
...
# NOT RUN IN AWS IMPLEMENTATION
rule GetPileupSummaries:
    input: 
        bam = rules.bqsr.output.bam,
        targets = idt_target_w100,
        gnomad = '/path/to/data/references/GRCh37_af-only-gnomad.raw.sites_chr.vcf',
    output: 
        table = join(outdir, '01_prepare_bam/GetPileupSummaries/{sample}_GetPileupSummaries.table'),
    shell: """
        # gnomad data from here
...
rule mutect2_TvN:
    input: 
        bams = lambda wildcards: expand(join(outdir, '01_prepare_bam/recalibrate/{sample}.fixchr.bam'), sample=pt_tp_to_samples[wildcards.patient_tp]),
        ref = REF_FILE,
        targets = idt_target_w100,
    output:
...
rule copy_TvN_vcf_remote:
    input: 
        ...
        model_f = rules.mutect2_TvN_readOrientation.output.model,
    output:
    ...
    shell: """
        ...
        cp {input.f1r2_uf} {output.f1r2_uf}
        cp {input.vcf_f} {output.vcf_f}
        cp {input.model_f} {output.model_f}
    """
...
rule copy_TvN_exome_vcf_remote:
    input: 
        ...
        model_f = rules.mutect2_TvN_exome_readOrientation.output.model,
    output:
